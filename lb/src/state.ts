...
static GRPC_SERVERS = process.env.GRPC_SERVERS?.split(';').filter(Boolean).map(address => ({ address, requestReceive: 0 }))  as unknown as IGRPC_SERVERS[]
static POOL: string[] = []
static connectionCounts = this.GRPC_SERVERS.map(server => server.address).reduce((acc, address) => {
    acc[address] = 0;
    return acc;
}, {} as any)
static latencies = this.GRPC_SERVERS.map(server => server.address).reduce((acc, address) => {
    acc[address] = { 
      ewma: 0,
